...
ngx_int_t ngx_event_connect_peer(ngx_peer_connection_t *pc)
{
    ...
    ngx_event_t       *rev, *wev;
    ngx_connection_t  *c;
    /* 通过Load-balance模块的get接口选择一个后端服务器，比如round_robin的 ngx_http_upstream_get_round_robin_peer()*/
    rc = pc->get(pc, pc->data);
    if (rc != NGX_OK) {
        return rc;
    }
    s = ngx_socket(pc->sockaddr->sa_family, SOCK_STREAM, 0);//建立TCP套接字
    ngx_log_debug1(NGX_LOG_DEBUG_EVENT, pc->log, 0, "socket %d", s);
    if (s == -1) {
        ngx_log_error(NGX_LOG_ALERT, pc->log, ngx_socket_errno,ngx_socket_n " failed");
        return NGX_ERROR;
    }
    c = ngx_get_connection(s, pc->log);//获取一个ngx_http_connection_t结构体
    if (c == NULL)
    {
    ...
    wev->own_lock = &c->lock;
#endif
    /*调用add_conn方法把TCP套接字以期待可读、可写事件的方式添加到事件搜集器中，如epoll，如果这个套接字上出现了预期的网络事件，则希望epoll能够
     回调它的handler方法*/
    if (ngx_add_conn) 
	{
