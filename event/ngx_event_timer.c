...
/* 找出红黑树中最左边的节点，如果它的超时时间大于当前时间，也就表明目前的定时器中
没有一个事件满足触发条件，这时返回这个超时与当前时间的差值，也就是需要经过多少毫秒
会有事件超时触发；如果它的超时时间小于或等于当前时间，则返回0，表示定时器中已经存在
超时需要触发的事件*/
...
/* 检查定时器中的所有事件，按照红黑树关键字由小到大的顺序依次调用已经满足超时
条件需要被触发事件的handler回调方法*/
