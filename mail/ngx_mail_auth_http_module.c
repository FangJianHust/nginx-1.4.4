...
{
	/* 保存着发往认证服务器的请求，它是根据解析客户端请求得到的ngx_mail_session_t，使用ngx_mail_auth_http_create_request
	方法构造出的内存缓冲区，这里的请求是一种类HTTP请求*/
    ngx_buf_t                      *request;
    ngx_buf_t                      *response;//保存认证服务器返回的类HTTP响应的缓冲区，缓冲区指向的内存大小固定为1KB
    ngx_peer_connection_t           peer;//nginx与认证服务器之间的连接

	/* 解析来自认证服务器类HTTP的响应行、头部的方法，默认为ngx_mail_auth_http_ignore_status_line */
    ngx_mail_auth_http_handler_pt   handler;

	/* 在使用状态机解析认证服务器返回的类HTTP响应时，使用state表示解析的状态 */
    ngx_uint_t                      state;
    ...
    ngx_str_t                       addr;//认证服务器返回的Auth-Server头部
    ngx_str_t                       port;//认证服务器返回的Auth-Port头部
    ngx_str_t                       err;//错误信息
    ngx_str_t                       errmsg;//错误信息构成的字符串
    ngx_str_t                       errcode;//错误码构成的字符串

	/* 认证服务器返回Auth-Wait头部时带的时间戳将被设置到sleep成员中，而nginx等待的时间也由sleep维护，sleep为0
	时将会设置quit标志位为1，表示请求非正常结束，把错误码返回给客户*/
    time_t                          sleep;

    ngx_pool_t                     *pool;//用于邮件认证的独立内存池，它的初始大小为2KB
};
...
static void ngx_mail_auth_http_write_handler(ngx_event_t *wev)
{
    ...
    c = wev->data;//写事件的data成员存放的是nginx与认证服务器间的TCP连接
    s = c->data;//连接的data成员存放的是ngx_mail_session_t结构体

    ctx = ngx_mail_get_module_ctx(s, ngx_mail_auth_http_module);//获得描述认证过程的ngx_mail_http_ctx_t结构体
    ...
    n = ngx_send(c, ctx->request->pos, size);//向认证服务器发送请求
    ...
    if (n > 0) 
	{
        ctx->request->pos += n;//更新request缓冲区

		/* size表示还需要发送的请求的长度，n表示本次发送的长度，当它们相等时，表示已经将全部响应发送到认证服务器 */
        if (n == size) 
		{
			/* 将nginx与认证服务器之间的写事件回调方法设置为任何事情都不做的方法 */
            wev->handler = ngx_mail_auth_http_dummy_handler;

			/* 由于不再需要发送请求，所以不再需要监控发送是否超时 */
            if (wev->timer_set) 
			{
                ngx_del_timer(wev);
            }
