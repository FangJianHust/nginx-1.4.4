...
static ngx_int_t ngx_http_proxy_handler(ngx_http_request_t *r)
{
...
#endif
    /*以下给upstream对象设置回调函数*/
    u->create_request = ngx_http_proxy_create_request;//ngx_http_upstream_init->ngx_http_upstream_init_request调用
    ...
    u->accel = 1;
    /*正式开始进行处理，首先读取客户端请求体，读取请求体后执行ngx_http_upstream_init方法，这意味着nginx把读取客户端上传文件的压力放在了自己
    这一边，后端服务器是感知不到的*/
...
static ngx_int_t ngx_http_proxy_process_status_line(ngx_http_request_t *r)
{
    ...
    u->process_header = ngx_http_proxy_process_header;//真正进行头部解析
