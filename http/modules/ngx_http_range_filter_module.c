...
static ngx_int_t ngx_http_range_header_filter(ngx_http_request_t *r)
{
    ...
    if (r->http_version < NGX_HTTP_VERSION_10
        ...
        /* 在前面的处理中，有些功能会执行ngx_http_clear_accept_ranges()宏设置allow_ranges为0，比如gzip功能 */
        || !r->allow_ranges)
    {
    ...
    //判断客户端是否在进行分块请求，并且分块请求的单位是否为bytes
...
parse:
    ...
    /* ngx_http_range_parse解析请求首部中指定的分块数据的范围*/
    switch (ngx_http_range_parse(r, ctx, clcf->max_ranges)) {

    case NGX_OK://解析成功
        ngx_http_set_ctx(r, ctx, ngx_http_range_body_filter_module);

        r->headers_out.status = NGX_HTTP_PARTIAL_CONTENT;//设置状态吗为206，表示部分请求成功
        r->headers_out.status_line.len = 0;

        //只有一个范围，比如Range:bytes=1-3
        if (ctx->ranges.nelts == 1) {
            return ngx_http_range_singlepart_header(r, ctx);
        }
        //多个范围，比如Range:bytes=1-3,4-8
...
/* 解析请求首部中指定的分块数据的范围，将解析出来的范围保存在ngx_http_range_t结构体中*/
