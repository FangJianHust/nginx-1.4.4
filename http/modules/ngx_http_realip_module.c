...
/*官方的ngx_http_realip_module模块是从NGX_HTTP_POST_READ_PHASE阶段介入处理HTTP请求的，它在
postconfiguration方法中把自定义的ngx_http_handler_pt处理方法添加到HTTP框架中的，这个
ngx_http_realip_init方法实际上就是postconfiguration接口的实现*/
static ngx_int_t ngx_http_realip_init(ngx_conf_t *cf)
{
    ngx_http_handler_pt        *h;
    ngx_http_core_main_conf_t  *cmcf;

	//首先获取全局的ngx_http_core_main_conf_t结构体
    cmcf = ngx_http_conf_get_module_main_conf(cf, ngx_http_core_module);

	/* phase数组中有11个成员，取出NGX_HTTP_POST_READ_PHASE阶段的handlers动态数组，向其中添加
	ngx_http_handler_pt处理方法，这样ngx_http_realip_module模块就介入HTTP请求的
	NGX_HTTP_POST_PHASE处理阶段了*/
    h = ngx_array_push(&cmcf->phases[NGX_HTTP_POST_READ_PHASE].handlers);
    if (h == NULL) {
        return NGX_ERROR;
    }

	//该方法就是实现了ngx_http_handler_pt接口的方法
    *h = ngx_http_realip_handler;

	/* 实际上，同一个HTTP模块的同一个ngx_http_realip_handler方法，完全可以设置到两个不同的阶段
	中，例如，phase[NGX_HTTP_PREACCESS_PHASE].handlers动态数组中叶添加了
	ngx_http_realip_handler方法*/
    h = ngx_array_push(&cmcf->phases[NGX_HTTP_PREACCESS_PHASE].handlers[NGX_HTTP_PREACCESS_PHASE].handlers);
    if (h == NULL) {
        return NGX_ERROR;
    }
