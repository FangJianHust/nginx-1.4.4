...
ngx_http_rewrite_if(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    ctx->loc_conf = ngx_pcalloc(cf->pool, sizeof(void *) * ngx_http_max_module);//鐢熸垚涓€涓暟缁勫瓨鍌ㄦ墍鏈夌殑HTTP妯″潡create_loc_conf鏂规硶杩斿洖鐨勫湴鍧€
    if (ctx->loc_conf == NULL) {
        return NGX_CONF_ERROR;
    }

    for (i = 0; ngx_modules[i]; i++) {//閬嶅巻鎵€鏈夌殑HTTP妯″潡
        ...
        if (module->create_loc_conf) {//濡傛灉杩欎釜HTTP妯″潡瀹炵幇浜哻reate_loc_conf锛屽氨璋冪敤瀹冿紝骞舵妸杩斿洖鐨勫湴鍧€瀛樺偍鍒發oc_conf涓?
