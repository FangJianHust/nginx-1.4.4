...
//HTTP框架式通过静态的二叉查找树来保存location的，这样可以加速location的查找
static ngx_int_t ngx_http_ssl_init(ngx_conf_t *cf)
{
    ngx_uint_t                   s;
    ngx_http_ssl_srv_conf_t     *sscf;
    ...
    cmcf = ngx_http_conf_get_module_main_conf(cf, ngx_http_core_module);//cmcf就是该http块下全局的ngx_http_core_main_conf_t结构体
    cscfp = cmcf->servers.elts;//cscfp指向保存所有ngx_http_core_srv_conf_t结构体指针的servers动态数组的第一个元素
	...
	{//遍历http块下的所有server块
        ...
        clcf = cscfp[s]->ctx->loc_conf[ngx_http_core_module.ctx_index];//clcf是server块下的ngx_http_core_loc_conf_t结构体，它的locations成员以双向链表关联着属于这个server块的所有location块对应的ngx_http_core_loc_conf_t结构体
