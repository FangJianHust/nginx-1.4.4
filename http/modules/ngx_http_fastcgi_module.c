...
static ngx_int_t ngx_http_fastcgi_handler(ngx_http_request_t *r)
{
    ...
    /*正式开始进行处理，首先读取客户端请求体，读取请求体后执行ngx_http_upstream_init方法，这意味着nginx把读取客户端
     * 上传文件的压力放在了自己这一边，后端服务器比如PHP引擎是感知不到的
     */
...
/* 一方面进行配置文件里相关配置变量的解析、传递参数的设置，另一方面还需从客户端与nginx之间的HTTP通信协议转换为nginx与
 * 后端服务器之间的fastcgi通信协议 */
static ngx_int_t ngx_http_fastcgi_create_request(ngx_http_request_t *r)
{
    off_t                         file_pos;
