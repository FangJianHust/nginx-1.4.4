...
//头过滤链上的第一个过滤模块，处理if-Modified-since和if-unmodified-since首部
...
static ngx_int_t ngx_http_not_modified_header_filter(ngx_http_request_t *r)
{
    ...
    if (r->headers_in.if_modified_since || r->headers_in.if_none_match) {
        ...
        //继续执行下一个过滤模块，但不能像if_unmodified_since失败那样，调用ngx_http_filter_finalize_request直接返回
        return ngx_http_next_header_filter(r);
    }
...
//判断在首部if_unmodified_since指定的时间以来，文件是否修改过。返回1表示没有修改过
static ngx_uint_t ngx_http_test_if_unmodified(ngx_http_request_t *r)
{
    time_t  iums;
...
//判断在首部if_modified_since指定的时间以来，文件是否修改过。返回1表示没有修改过
static ngx_uint_t ngx_http_test_if_modified(ngx_http_request_t *r)
{
    time_t                     ims;
...
//把ngx_http_not_modified_header_filter过滤方法添加到过滤链表头，并保存原来的表头方法
static ngx_int_t ngx_http_not_modified_filter_init(ngx_conf_t *cf)
{
