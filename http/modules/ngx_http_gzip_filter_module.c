...
/*在服务器发出的HTTP响应包中，这个模块可以按照配置文件制定的conntent-type对特定大小的HTTP响应包体执行gzip压缩*/
...
static ngx_int_t ngx_http_gzip_header_filter(ngx_http_request_t *r)
{
    ...
    if (!conf->enable
        ...
        || ngx_http_test_content_type(r, &conf->types) == NULL//判断响应内容是否是需要压缩的类别，如text/html等
        || r->header_only)
    {
    ...
    if (!r->gzip_tested)
    {
    	//判断客户端是否支持解压缩，根据客户端的首部Accept-Encoding
        if (ngx_http_gzip_ok(r) != NGX_OK)
        {
            return ngx_http_next_header_filter(r);//不支持则进入下一个头部过滤模块
        }
    ...
    /* 经过层层验证后，已经允许对响应体进行压缩*/

    //将 ctx保存下来，响应体压缩时通过它来判断是否需要进行响应体的压缩
...
//该模块既包含有头过滤链上的回调函数，又包含有体过滤链上的过滤函数
static ngx_int_t ngx_http_gzip_filter_init(ngx_conf_t *cf)
{
