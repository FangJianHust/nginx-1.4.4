...
参数b:header_in缓冲区
返回值:NGX_OK表示成功地解析到完整的HTTP请求行；NGX_AGAIN表示目前接收到的字符流不足以
构成完整的请求行，还需要接收更多的字符流；返回NGX_HTT_PARSE_INVALID_REQUEST或者
NGX_HTTP_PARSE_INVALID_09_METHOD等其他值表示接收到非法的请求行*/
ngx_int_t ngx_http_parse_request_line(ngx_http_request_t *r, ngx_buf_t *b)
{
    ...
    return NGX_AGAIN;//目前接收到的字符流不足以构成完整的请求行，还需要接收更多的字符流
...
/* 解析HTTP头部。返回NGX_OK表示解析出一行HTTP头部；返回NGX_HTTP_PARSE_HEADER_DONE表示解析出了
完整的HTTP头部；返回NGX_AGAIN表示还需要接收更多的字符流才能够继续解析*/
...
////进行chunked解码，获得每一个chunked块的大小和内容
