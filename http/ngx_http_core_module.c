...
void ngx_http_handler(ngx_http_request_t *r)
{
    ...
    if (!r->internal) 
	{
        ...
        r->lingering_close = (r->headers_in.content_length_n > 0 || r->headers_in.chunked);//延迟关闭
        r->phase_handler = 0;//意味着从ngx_http_phase_engine_t指定数组的第一个回调方法开始执行
    } 
	/*如果为internal标志位1，表示请求当前需要做内部跳转，将要把结构体中的phase_handler序号设置为server_rewrite_index,这意味着无论之前
	执行到哪一个阶段，马上都要重新从NGX_HTTP_SERVER_REWRITE_PHASE阶段开始再次执行，这是nginx的请求可以反复rewrite重定向的基础*/
	else 
	{
    ...
    r->gzip_vary = 0;
#endif
	//类似于read_event_handler方法
    r->write_event_handler = ngx_http_core_run_phases;
	//开始执行write_event_handler方法
...
void ngx_http_core_run_phases(ngx_http_request_t *r)
{
	...
	/* 调用每个ngx_http_phase_handler_t处理阶段的checker方法，而不会执行handler方法，这是因为handler方法其实仅能在checker方法中被
	调用，而且checker方法由HTTP框架实现，所以可以控制各HTTP模块实现的处理方法在不同的阶段中采用不同的调用行为*/
...
/*当HTTP框架在建立TCP连接上接收到客户发送的完整HTTP请求头部时，开始执行NGX_HTTP_POST_READ_PHASE阶段的checker方法。
有3个HTTP阶段都是用了该方法作为它们的checker方法：NGX_HTTP_POST_READ_PHASE、NGX_HTTP_PREACCESS_PHASE、NGX_HTTP_LOG_PHASE*/
ngx_int_t ngx_http_core_generic_phase(ngx_http_request_t *r, ngx_http_phase_handler_t *ph)
{
    ...
    rc = ph->handler(r);
	/*如果handler方法返回NGX_OK,则执行下一个ngx_http_phase阶段中的第一个ngx_http_handler_pt处理方法，即使当前阶段中后续还有一些HTTP
	模块设置了ngx_http_handler_pt处理方法，返回NGX_OK之后它们也是得不到执行的；如果下一个ngx_http_phases阶段中没有任何HTTP模块设置
	了ngx_http_handler_pt处理方法，将再次寻找之后的阶段，如此循环下去*/
    if (rc == NGX_OK) 
	{
        r->phase_handler = ph->next;
        return NGX_AGAIN;
    }
	/*如果返回NGX_DECLINED，那么将按照顺序进入下一个处理方法，这个处理方法既可能属于当前阶段，也可能属于下一个阶段；这个顺序就是
	ngx_hhttp_phase_engine_t中所有ngx_http_phase_handler_t结构体组成的数组的顺序*/
    if (rc == NGX_DECLINED)
	{
        r->phase_handler++;
        return NGX_AGAIN;
    }
	/*如果返回NGX_AGAIN或者NGX_DONE，则表示当前的ngx_http_handler_pt处理方法尚未结束，这意味着该处理方法在当前阶段有机会再次被调用，
	这时一般会把控制权交还给事件模块，当下次可写事件发生时会再次执行到该ngx_http_handler_pt处理方法*/
    if (rc == NGX_AGAIN || rc == NGX_DONE) 
	{
        return NGX_OK;
    }
	//如果返回NGX_ERROR或者NGX_HTTP_开头的返回码，则调用ngx_http_finalize_request结束请求
    ngx_http_finalize_request(r, rc);
    return NGX_OK;
}

/*该方法充当了用于重写URL的NGX_HTTP_SERVER_REWRITE_PHASE和NGX_HTTP_REWRITE_PHASE这两个阶段的checker方法*/
ngx_int_t ngx_http_core_rewrite_phase(ngx_http_request_t *r, ngx_http_phase_handler_t *ph)
{
	...
	//调用当前阶段的HTTP模块实现的handler方法
    rc = ph->handler(r);
	//当前处理方法执行完毕，按照顺序执行下一个ngx_http_handler_pt处理方法
    if (rc == NGX_DECLINED) 
	{
        r->phase_handler++;
        return NGX_AGAIN;
    }
	/* handler方法返回NGX_DONE意味着刚才的handler方法无法在这一次调度中处理完这一阶段，它需要多次的调度才能完成，此时返回NGX_OK，它会
	使得HTTP框架立刻把控制权交个epoll等事件模块，不再处理当前请求，唯有这个请求上的事件再次被触发时才会继续执行*/
    if (rc == NGX_DONE) 
	{
        return NGX_OK;
    }
...
/*执行NGX_HTTP_FIND_CONFIG_PHASE阶段,任何HTTP模块不可以向这一阶段添加处理方法。该方法实际上就是根据NGX_HTTP_SERVER_REWRITE_PHASE
步骤重写后的URI检索出匹配location块的，其原理为从location组成的静态二叉查找树中快速检索*/
...
/*NGX_HTTP_POST_REWRITE_PHASE阶段只能由HTTP框架实现，不允许HTTP模块向该阶段添加
ngx_http_handler_pt处理方法。本方法是NGX_HTTP_POST_REWRITE_PHASE阶段的checker
方法，它的意义在于检查rewrite重写URI的次数不可以超过10次，以防止由于rewrite死
循环而造成整个nginx服务都不可用*/
...
/*该方法用于NGX_HTTP_ACCESS_PHASE阶段的处理方法，这一阶段用于控制用户发起的请求是否合法，如检测客户端的IP地址是否允许访问，它涉及
nginx.conf配置文件中satisfy配置项的参数值*/
ngx_int_t ngx_http_core_access_phase(ngx_http_request_t *r, ngx_http_phase_handler_t *ph)
{
    ngx_int_t                  rc;
    ngx_http_core_loc_conf_t  *clcf;
	/*既然NGX_HTTP_ACCESS_PHASE阶段用于控制客户端是否有权限访问服务，那么它就不需要对子请求起作用，如果当前请求是一个派生的子请求*/
    if (r != r->main) 
	{	
		/*子请求不需要执行NGX_HTTP_ACCESS_PHASE阶段的处理方法的，直接将phase_handler设为下一个阶段的处理方法的序号*/
        r->phase_handler = ph->next;
		//返回AGAIN是希望HTTP框架立刻执行新的HTTP阶段的请求
        return NGX_AGAIN;
    }
    ngx_log_debug1(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,"access phase: %ui", r->phase_handler);
	//如果当前请求是来自客户端的原始请求，调用HTTP模块在这一阶段中实现的handler方法
    rc = ph->handler(r);
	/*意味着handler方法执行完毕且意犹未尽，希望立刻执行下一个handler方法，无论其是否属于NGX_HTTP_ACCESS_PHASE阶段*/
    if (rc == NGX_DECLINED) 
	{
        r->phase_handler++;
        return NGX_AGAIN;
    }
	/*当前的NGX_HTTP_ACCESS_PHASE阶段没有一次性处理完毕，所以在这一步中会暂时结束当前请求，将控制权交给事件模块，当请求对应的事件再次触发
	时才会继续处理该请求*/
    if (rc == NGX_AGAIN || rc == NGX_DONE) 
	{
        return NGX_OK;
	...
	/*由于NGX_HTTP_ACCESS_PHASE是在NGX_HTTP_FIND_CONFIG_PHASE阶段之后的，因此这时已经找到了匹配的location配置块，把location对应
	的ngx_http_core_loc_conf_t取出来*/
    clcf = ngx_http_get_module_loc_conf(r, ngx_http_core_module);
	/*意味着NGX_HTTP_ACCESS_PHASE阶段的handler方法必须共同作用于这个请求，这时handler方法的返回值就具有不同的意义了*/
	if (clcf->satisfy == NGX_HTTP_SATISFY_ALL) 
	{	
		/*返回OK意味着这个handler方法所在的HTTP模块认为当前请求时具备访问权限的，需要再次检查NGX_HTTP_ACCESS_PHASE阶段的下一个HTTP
		模块的handler方法*/
        if (rc == NGX_OK) 
		{
            r->phase_handler++;
	...
	/*如果satistfy为NGX_HTTP_SATISFY_ANY，也就是说并不强制要求NGX_HTTP_ACCESS_PHASE阶段的所有handler方法必须同时起作用，那么这时
	handler方法的返回值又具有了不同的意义*/
	else
	{
		/*返回OK表示handler方法认为这个请求有权访问服务，而且不用再调用NGX_HTTP_ACCESS_PHASE阶段的其它handler方法，将执行下一个
		ngx_http_phases阶段中的第一个ngx_http_handler_pt处理方法*/
        if (rc == NGX_OK) 
		{
            r->access_code = 0;
		...
		/*表示这个HTTP模块的handler方法认为请求没有权限访问服务，但只要NGX_HTTP_ACCESS_PHASE阶段的任何一个handler方法返回OK就认为
		请求合法，所以后续的handler方法可能会更该这一结果*/
...
/* NGX_HTTP_POST_ACCESS_PHASE(不允许HTTP模块向该阶段添加ngx_http_handler_pt方法)阶段完全是为了之前的NGX_HTTP_ACCESS_PHASE阶段服务
的，如果没有任何HTTP模块介入NGX_HTTP_ACCESS_PHASE阶段，NGX_HTTP_POST_ACCESS_PHASE就不会存在。本方法是NGX_HTTP_POST_ACCESS_PHASE
阶段的checker方法，它检查ngx_http_request_t请求中的access_code成员，当其不为0时就结束请求(表示没有访问权限)，否则继续执行下一个
...
/* NGX_HTTP_TRY_FILES_PHASE(不允许HTTP模块向该阶段添加ngx_http_handler_pt)的checker方法
，该方法与nginx.conf中的try_files配置项密切相关的，如果try_filse后指定的静态文件资源中
有一个可以访问，这时就会直接读取文件并发送响应给用户，不会再执行后续的阶段；如果所有的
静态文件资源都无法执行，将会继续执行ngx_http_phase_engine_t中的下一个ngx_http_handler_pt
处理方法*/
...
/*该方法是NGX_HTTP_CONTENT_PHASE阶段的checker方法，开发HTTP模块时最常用的一个阶段，该阶段用于真正处理请求内容*/
ngx_int_t ngx_http_core_content_phase(ngx_http_request_t *r,ngx_http_phase_handler_t *ph)
{
    size_t     root;
    ngx_int_t  rc;
    ngx_str_t  path;
	/* 如果content_handler成员为空，则开始执行全局有效的handler方法，否则执行content_handler方法，在NGX_HTTP_FIND_CONFIG_PHASE
	阶段会把它设置为匹配乐URI的location块中对应的ngx_http_core_loc_conf_t结构体的handler成员，如果有的话 */
    if (r->content_handler) 
	{
		/* 首先设置ngx_http_request_t结构体的write_event_handler成员为不做任何事情，也就是告诉HTTP框架再有可写事件时就直接调用
		ngx_request_empty_handler直接把控制权交给事件模块，这是因为HTTP框架在这一阶段调用HTTP模块处理请求就意味着接下来只希望该模
		块处理请求，不希望其它HTTP模块还在此时处理可写事件、向客户端发送响应*/
        r->write_event_handler = ngx_http_request_empty_handler;
		/* 调用content_handler方法处理请求，并把它的返回值作为参数传递给ngx_http_finalize_request方法来结束请求，该方法会根据引用
		计数来决定自己的行为*/
        ngx_http_finalize_request(r, r->content_handler(r));
        return NGX_OK;
    }
    ngx_log_debug1(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,"content phase: %ui", r->phase_handler);
	//在没有content_handler方法时，会根据phase_handler序号调用handler处理方法
    rc = ph->handler(r);
	//意味着不再执行该阶段的其他handler方法
    if (rc != NGX_DECLINED) 
	{
        ngx_http_finalize_request(r, rc);
        return NGX_OK;
    ...
    /* rc == NGX_DECLINED */
    ph++;
	//检查当前方法是否是最后一个handler方法，如果不是，则把请求序号加1
    if (ph->checker) 
	{
        r->phase_handler++;
        return NGX_AGAIN;
    }
    /* 已经没有handler方法了，则结束请求，但需要根据URI确定返回什么样的HTTP响应。这里可以看到，在请求第10个阶段NGX_HTTP_CONTETN_PHASE后,
    并没有调用第11个阶段NGX_HTTP_LOG_PHASE的处理方法，通过比较11.6节的其他checker方法，就会发现与之前的方法都不同，事实上，记录访问日志是
    必须在请求将要结束时进行的，因此，NGX_HTTP_LOG_PHASE阶段的回调方法在11.10.2节介绍的ngx_http_free_request方法中才会调用 */
...
//从静态二叉查找树中快速检索到ngx_http_core_conf_t结构体
static ngx_int_t ngx_http_core_find_location(ngx_http_request_t *r)
{
    ngx_int_t                  rc;
    ...
    //首先进行静态匹配，也就是绝对匹配和前缀匹配
    rc = ngx_http_core_find_static_location(r, pclcf->static_locations);

    //返回NGX_AGAIN,则再进行前缀匹配
    if (rc == NGX_AGAIN)
    {
#if (NGX_PCRE)
        clcf = ngx_http_get_module_loc_conf(r, ngx_http_core_module);
...
ngx_int_t ngx_http_send_header(ngx_http_request_t *r)
{
	...
	/*调用头部过滤模块中的方法，其中最后一个是头部过滤模块ngx_http_header_filter_module的ngx_http_header_filter方法*/
    return ngx_http_top_header_filter(r);
}

//该方法用于发送响应包体，参数in存放响应包体的缓冲区
ngx_int_t ngx_http_output_filter(ngx_http_request_t *r, ngx_chain_t *in)
{
	...
	/*调用过滤包体模块中的方法，其中最后一个是过滤模块ngx_http_write_module的ngx_http_write_filter方法*/
...
/* 参数r为当前的请求，uri和args为新的要发起的uri和args，当然args可以为NULL，psr为指向一个ngx_http_request_t指针的指针，
它的作用就是获得创建的子请求，ps的类型为ngx_http_post_subrequest_t，它的定义如下：*/
ngx_int_t ngx_http_subrequest(ngx_http_request_t *r,ngx_str_t *uri, ngx_str_t *args, ngx_http_request_t **psr,
		ngx_http_post_subrequest_t *ps, ngx_uint_t flags)
{
    ngx_time_t                    *tp;
    ngx_connection_t              *c;
    ngx_http_request_t            *sr;//临时变量，子请求
    ngx_http_core_srv_conf_t      *cscf;
    ...
    sr->method = NGX_HTTP_GET;//子请求的method只能是GET
    ...
    sr->main = r->main;//子请求的main字段都指向主请求
    sr->parent = r;
    ...
    /* 写事件的回调函数设置为ngx_http_handler意味着对该子请求的处理将完全重走一遍处理逻辑 */
    ...
    /* 封装结构体ngx_http_postponed_request_t，并将其添加到父请求的postponed链表内 */
    ...
    *psr = sr;
    ...
    /* 把子请求加入到主请求所对应的ngx_http_request_t结构体的posted_requests字段内 */
...
/* 在解析main级别配置项时，如果发现了server{}配置项，就会调用ngx_http_core_server方法，该方法属于ngx_http_core_module模块
(第一个HTTP模块)，而在这个方法里则会开始解析srv级别的配置项*/
static char * ngx_http_core_server(ngx_conf_t *cf, ngx_command_t *cmd,
															  void *dummy)
{
    char                        *rv;
	...
	//指向所属的http块下ngx_http_conf_ctx_t结构体的main_conf指针数组，而srv_conf和loc_conf将重新分配指针数组
    ...
    for (i = 0; ngx_modules[i]; i++) 
	{
		...
		/*循环调用所有HTTP模块的create_srv_conf方法，将返回的结构体指针按照模块序号
		ctx_index保存到上述的srv_conf指针数组中*/
		...
		/*循环调用所有HTTP模块的create_loc_conf方法，将返回的结构体指针按照模块序号
		ctx_index保存到上述的loc_conf指针数组中*/	
    ...
    /* 第1个HTTP模块就是ngx_http_core_module模块，它在create_srv_conf方法中将会生成非常
    关键的ngx_http_core_srv_conf_t配置结构体，这个结构体对应着当前正在解析的server块，
    这时，将ngx_http_core_srv_conf_t添加到全局的ngx_http_core_main_conf_t结构体的servers
    动态数组中*/
    ...
    *cf = pcf;
	...
	/*如果在server{}块内没有解析到listen配置项，则意味着当前的server虚拟主机并没有监听
	TCP端口，这不符合HTTP框架的设计原则，于是将开始监听默认端口80，实际上，如果当前进
	程没有权限监听1024以下的端口，则会改为监听8000端口*/
...
/* 在解析srv级别配置项时，如果发现了location{}配置块，就会回调ngx_http_core_location方
法，该方法属于ngx_http_core_module模块(第1个HTTP模块),在这个方法里则会开始解析loc级别
的配置项*/
static char * ngx_http_core_location(ngx_conf_t *cf, ngx_command_t *cmd, void *dummy)
{
	...
	//这里的main_conf和srv_conf都将指向所属的server块下ngx_http_conf_ctx_t结构体的main_conf和srv_conf指针数组
    ctx->main_conf = pctx->main_conf;
    ctx->srv_conf = pctx->srv_conf;

	//loc_conf则指向重新分配的指针数组
    ctx->loc_conf = ngx_pcalloc(cf->pool, sizeof(void *) * ngx_http_max_module);
    if (ctx->loc_conf == NULL) {
        return NGX_CONF_ERROR;
    }
	...
	/* 第1个HTTP模块是ngx_http_core_module模块，它在create_loc_conf方法中将会生成
	ngx_http_core_loc_conf_t配置结构体，可以认为该结构体对应着当前解析的location块。
	这时会生成ngx_http_location_queue_t结构体，因为每一个ngx_http_core_conf_t结构体
	都对应着1个ngx_http_location_queue_t，因此，此处将把ngx_http_location_queue_t串
	联成双向链表*/
...
/*配置项listen的处理函数，主要就是解析listen指令并将对应的结果保存的变量lsopt内，最后调用ngx_http_add_listen(),该函数将所有的配置以
[port,addr]的形式组织在http核心配置ngx_http_core_main_conf_t.ports数组内*/
