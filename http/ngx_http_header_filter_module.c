...
/* 该方法是最后一个头部过滤模块ngx_http_header_filter_module提供的方法，它会根据HTTP规则把headers_out中的成员变量序列化为字符流发送出去 */
static ngx_int_t ngx_http_header_filter(ngx_http_request_t *r)
{
    u_char                    *p;
    size_t                     len;
	...
	//为了防止反复的发送响应头部
    r->header_sent = 1;
	//如果当前请求只是一个子请求，它是不存在发送HTTP响应这个概念的
    if (r != r->main) 
	{
        return NGX_OK;
    }
    ...
    *b->last++ = CR; *b->last++ = LF;
    ...
    out.buf = b;
    out.next = NULL;
	//调用ngx_http_write_filter方法来发送响应头部
