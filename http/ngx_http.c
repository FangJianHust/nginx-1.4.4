...
static char * ngx_http_block(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    *(ngx_http_conf_ctx_t **) conf = ctx;
    ngx_http_max_module = 0;//记录http模块的个数
	/*按照在ngx_modules数组中的顺序，由0开始依次递增地设置所有HTTP模块的ctx_index字段，这个字段将决定HTTP模块应用于请求时的顺序*/
    ...
    for (m = 0; ngx_modules[m]; m++) 
	{
		...
		/*依次调用所有HTTP模块的create_main_conf方法，产生的配置结构体指针将按照各模块ctx_index字段指定的顺序放入ngx_http_conf_ctx_t
		 结构体的main_conf数组中*/
		...
		/*依次调用所有HTTP模块的create_srv_conf方法，产生的配置结构体指针将按照各模块ctx_index字段指定的顺序放入ngx_http_conf_ctx_t
		结构体的srv_conf数组中*/
		...
		/*依次调用所有HTTP模块的create_loc_conf方法，产生的配置结构体指针将按照各模块ctx_index字段指定的顺序放入ngx_http_conf_ctx_t
		 结构体的loc_conf数组中*/
    ...
    for (m = 0; ngx_modules[m]; m++) 
	{
        ...
        module = ngx_modules[m]->ctx;
		//依次调用所有HTTP模块的preconfiguration方法
    ...
    /*解析http{}块下的main级别配置项,在解析main级别配置项时，如果遇到server{}配置块，将会触发ngx_http_core_server方法解析server{}级别
    的配置项，在解析server级别的配置项时，如果遇到location{}配置块，将会触发ngx_http_core_location方法，并开始解析location级别的配置项 */
    cf->module_type = NGX_HTTP_MODULE;
    cf->cmd_type = NGX_HTTP_MAIN_CONF;
    rv = ngx_conf_parse(cf, NULL);//解析http{}
    if (rv != NGX_CONF_OK) {
        goto failed;
    }
	...
	//指向保存所有ngx_http_core_srv_conf_t结构体指针的servers动态数组的第一个元素
    ...
    for (m = 0; ngx_modules[m]; m++) 
	{
		...
		//调用ngx_http_merge_servers方法合并配置项
        rv = ngx_http_merge_servers(cf, cmcf, module, mi);
        if (rv != NGX_CONF_OK) {
            goto failed;
        }
    }
    /* 创建由location块构造的静态二叉平衡树，为了加快location的快速检索 */
    for (s = 0; s < cmcf->servers.nelts; s++) 
	{
		/* clcf是server块下的ngx_http_core_loc_conf_t结构体，10.2.3节曾经介绍过它的locations成员以双向链表关联着隶属于这个server
		块的所有location块对应的ngx_http_core_loc_conf_t结构体*/
        clcf = cscfp[s]->ctx->loc_conf[ngx_http_core_module.ctx_index];
		/* 将ngx_http_core_loc_conf_t组成的双向链表按照location匹配字符串进行排序，注意:这个操作是递归进行的，如果某个location块下
		还有其他location，那么他的locations链表也会被排序*/
        if (ngx_http_init_locations(cf, cscfp[s], clcf) != NGX_OK) 
		{
            return NGX_CONF_ERROR;
        }
		/* 根据已经按照location字符串排序过的双向链表，快速地构建静态的二叉查找树，与ngx_http_init_location方法类似，这个操作也是递归
		进行的*/
	...
	/* 有7个HTTP阶段是允许任何一个HTTP模块实现自己的ngx_http_handler_pt处理方法，并将其加入到这7个阶段中区的，在调用postconfiguration
	方法向这7个阶段中添加处理方法前，需要先将phases数组中这7个阶段里的handlers动态数组初始化,通过调用ngx_http_init_phases方法来初始化这
	7个动态数组*/
    ...
    for (m = 0; ngx_modules[m]; m++) 
	{
        ...
        module = ngx_modules[m]->ctx;
		/*依次调用所有HTTP模块的postconfiguration方法，HTTP模块可以在这一步骤中将自己的ngx_http_handler_pt处理方法添加到以上7个HTTP
		阶段中*/
	...
	//遍历phases数组中的处理方法，构造由所有处理方法构成的有序的phase_engine.handlers数组
    ...
    *cf = pcf;
	//构造server虚拟主机构成的支持通配符的散列表,参见10.4
    if (ngx_http_init_phase_handlers(cf, cmcf) != NGX_OK) 
	{
        return NGX_CONF_ERROR;
    }
    /* 构造监听端口与server间的关联关系，设置新连接事件的回调方法为ngx_http_init_connection,参见10.3*/
...
/* 在HTTP框架的初始化过程中，任何HTTP模块都可以在ngx_http_module_t接口的postconfiguration方法中将自定义的方法添加到handler
 动态数组中，这样，这些方法就会最终添加到ngx_http_core_main_conf_t.​phase_engine中​。然后由ngx_http_init_phase_handlers
 方法对​ngx_http_core_main_conf_t.phase_engine数组进行重组，将这些回调函数添加到
 ngx_http_core_main_conf_t.phase_engine.ngx_http_phase_handler_t数组​中​​​，而该数组的每一个成员包含一个checker方法和
 一个handler方法，且该数组中的成员按顺序对应于11个阶段中的checker方法和hander方法，当然，每个阶段可能有0到多个成员与之对应 */
...
static char * ngx_http_merge_servers(ngx_conf_t *cf, ngx_http_core_main_conf_t *cmcf,ngx_http_module_t *module, ngx_uint_t ctx_index)
{
	...
	//注意这个ctx是在http{}块下的全局ngx_http_conf_ctx_t结构体
    ...
    for (s = 0; s < cmcf->servers.nelts; s++) 
	{
		...
		{
			/*在这里，saved.srv_conf[ctx_index]参数是当前HTTP模块在http{}块下由create_srv_conf
			方法创建的结构体，而cscfp[s]->ctx->srv_conf[ctx_index]参数则是在server{}块下由
		...
		//如果当前模块实现了merge_loc_conf，则再调用合并方法
        if (module->merge_loc_conf) 
		{
            /* merge the server{}'s loc_conf */

			/*cscfp[s]->ctx->loc_conf这个动态数组中的成员都是server{}块下所有HTTP模块的
			...
			/*clcf时server块下ngx_http_core_module模块使用create_loc_conf方法产生的
			ngx_http_core_loc_conf_t结构体，它的locations成员将以双向链表的形式关联到所有当前
			server{}块下的location块*/
            clcf = cscfp[s]->ctx->loc_conf[ngx_http_core_module.ctx_index];
...
/*每个server块ngx_http_core_loc_conf_t中的locations双向链表会包含所属的全部location块，遍历它
可以合并srv、loc级别配置项*/
static char * ngx_http_merge_locations(ngx_conf_t *cf, ngx_queue_t *locations,void **loc_conf, ngx_http_module_t *module, ngx_uint_t ctx_index)
{
    ...
    for (q = ngx_queue_head(locations);q != ngx_queue_sentinel(locations);q = ngx_queue_next(q))
    {
		...
		/*如果location后的匹配字符串不依靠nginx自定义的通配符就可以完全匹配的话，则exact指向
		当前location对应的ngx_http_core_loc_conf_t结构体，否则使用Inclusive指向该结构体，且
		exact的优先级高于inclusive*/
        clcf = lq->exact ? lq->exact : lq->inclusive;

		/*clcf->loc_conf这个指针数组里保存着当前location下所有HTTP模块使用create_loc_conf方法
		生成的结构体指针*/
		...
		//因为location{}中可嵌套location{}配置块，所以可以继续合并的
...
/* 将ngx_http_core_loc_conf_t组成的双向链表按照location匹配字符串进行排序，注意:这个
操作是递归进行的，如果某个location块下还有其他location，那么他的locations链表也会被
排序*/
...
/* 根据已经按照location字符串排序过的双向链表，快速地构建静态的二叉查找树，与
ngx_http_init_location方法类似，这个操作也是递归进行的*/
static ngx_int_t ngx_http_init_static_location_trees(ngx_conf_t *cf,
    ngx_http_core_loc_conf_t *pclcf)
{
