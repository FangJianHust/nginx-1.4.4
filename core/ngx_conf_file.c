...
/**
 * 用来解析命令行传递的配置,即解析nginx命令行参数’-g’加入的配置
 */
...
/*
 * 解析配置项，ngx_conf_parse函数支持三种不同的解析环境：
	parse_file：解析配置文件；
...
char * ngx_conf_parse(ngx_conf_t *cf, ngx_str_t *filename)
{
    ...
    for ( ;; )
    {
        rc = ngx_conf_read_token(cf);//词法分析通过ngx_conf_read_token()函数完成
        ...
        if (cf->handler)
        {
            ...
            rv = (*cf->handler)(cf, NULL, cf->handler_conf);//其中handler是自定义解析函数指针
            if (rv == NGX_CONF_OK) {
                continue;
            }
...
{
    char  *p = conf;//获取当前配置结构体信息
    ngx_str_t        *field, *value;
    ngx_conf_post_t  *post;
    field = (ngx_str_t *) (p + cmd->offset);//当前配置结构体中配置项的位置
    if (field->data) {
        return "is duplicate";
    }
    value = cf->args->elts;
    *field = value[1];//将解析出来的值赋给该配置项中的相应成员
    if (cmd->post)
    {
        post = cmd->post;
...
char * ngx_conf_set_num_slot(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
	...
	/*根据ngx_command_t中的offset偏移量，可以找到结构体中的成员，而对于ngx_conf_set_num_slot方法而言，存储数字的必须是ngx_int_t类型，知道了
	 * 偏移量，又知道了类型，即可获得该参数.虽然预定义的配置项处理方法并不知道每个HTTP模块所定义的结构体包括哪些成员，但是由于从ngx_command_t
	 * 结构体中可以知道偏移量offset，再加上每种配置项解析方法都只能解析特定类型的数据结构，自然就可以通用的访问所有的模块参数了*/
    np = (ngx_int_t *) (p + cmd->offset);
    if (*np != NGX_CONF_UNSET) {
        return "is duplicate";
    }

    value = cf->args->elts;//value指向配置项的参数
    *np = ngx_atoi(value[1].data, value[1].len);//将字符串的参数转化为整型，并设置到create_lic_conf等方法生成的结构体的相关成员上
    if (*np == NGX_ERROR) {
        return "invalid number";
    }
    //如果ngx_command_t中的post已经实现，还需要调用post->post_handler方法
    if (cmd->post)
    {
        post = cmd->post;
