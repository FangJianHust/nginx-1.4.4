...
/* 搴旂敤鍦烘櫙锛氬綋鎻掑叆鑺傜偣鐨勫叧閿瓧瀛樺湪鏃讹紝涓嶈鐩栧師鏈夌殑鍚屽悕鍏抽敭瀛楄妭鐐癸紝鑰屾槸浣滀负鏂版彃鍏ョ殑鑺傜偣瀛樺湪銆傝妭鐐圭殑鏍囪瘑绗︽槸瀛楃涓?绾㈤粦鏍戠殑绗竴鎺掑簭渚濇嵁鏄叧閿瓧锛岀浜屾帓搴忎緷鎹槸瀛楃涓?*/
void ngx_str_rbtree_insert_value(ngx_rbtree_node_t *temp,ngx_rbtree_node_t *node, ngx_rbtree_node_t *sentinel)
{
    ...
    for ( ;; ) {
		...
		//棣栧厛姣旇緝key鍏抽敭瀛楋紝绾㈤粦鏍戜腑浠ey浣滀负绗竴绱㈠紩鍏抽敭瀛?        if (node->key != temp->key)
		{
		...
		{//褰搆ey鍏抽敭瀛楃浉鍚屾椂锛屼互瀛楃涓查暱搴︿负绗簩绱㈠紩鍏抽敭瀛?
            p = (n->str.len < t->str.len) ? &temp->left : &temp->right;

        } 
		else
		{//瀛楃涓查暱搴︿篃鐩稿悓鏃讹紝缁х画姣旇緝瀛楃涓插唴瀹?            p = (ngx_memcmp(n->str.data, t->str.data, n->str.len) < 0)
                 ? &temp->left : &temp->right;
        }

        if (*p == sentinel) 
		{//濡傛灉褰撳墠鑺傜偣P鏄摠鍏佃妭鐐癸紝閭ｄ箞璺冲嚭寰幆鍑嗗鎻掑叆鑺傜偣
            break;
        }

        temp = *p;//p鑺傜偣涓庤鎻掑叆鐨勮妭鐐瑰叿鏈夌浉鍚岀殑鏍囪瘑绗︽椂锛屽繀椤昏鐩栧唴瀹?    }

    *p = node;//鎻掑叆鑺傜偣
    node->parent = temp;
    node->left = sentinel;
    node->right = sentinel;
    ngx_rbt_red(node);//灏嗚妭鐐归鑹茶缃负绾㈣壊銆傛敞鎰忥紝绾㈤粦鏍戠殑ngx_rbtree_insert鏂规硶浼氱埍鍙兘鐨勬棆杞悗閲嶇疆璇ヨ妭鐐圭殑棰滆壊
}
