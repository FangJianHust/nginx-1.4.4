...
static ngx_core_module_t  ngx_core_module_ctx = {//定义ngx_core_module模块上下文
    ngx_string("core"),
...
ngx_module_t  ngx_core_module = {
    NGX_MODULE_V1,
    &ngx_core_module_ctx,                  /* 模块上下文m*/
    ngx_core_commands,                     /* 模块的命令 */
...
int ngx_cdecl main(int argc, char *const *argv)
{
	...
	//初始化日志，如初始化全局变量ngx_prefix，打开日志文件ngx_log_file.fd
	...
	/*创建一个临时的ngx_cycle_t变量，用它的成员存储配置文件的路径，事实上还用到了这个临时
	变量的其它成员，如log成员会指向屏幕输出的日志*/
    ngx_memzero(&init_cycle, sizeof(ngx_cycle_t));
    init_cycle.log = log;
    ngx_cycle = &init_cycle;

	//为ngx_cycle.pool创建大小为1024B的内存池
	...
	//初始化临时ngx_cycle_t的prefix, conf_prefix, conf_file, conf_param等配置文件路径参数
	...
	//继承sockets:Nginx在不重启服务升级时，也就是我们说过的平滑升级(1.9节)，它会不重启master进程而启动新版本的Nginx程序。这样，旧版本的master进程会通过execve系统调用来启动新版本的master，这时旧版本的master进程必须要通过一种方式告诉新版本的master进程这时在平滑升级，并且传递一些必要的信息。Nginx是通过环境变量来传递这些信息的，新版本的master进程通过ngx_add_inherited_sockets方法由环境变量里读取平滑升级信息，并对旧版本Nginx服务监听的句柄做继承处理
    ...
    ngx_max_module = 0;
	//初始化每个module的index为ngx_modules数组的下标，ngx_max_module为模块的个数
    for (i = 0; ngx_modules[i]; i++) 
	{
        ngx_modules[i]->index = ngx_max_module++;
    }

	//该初始化主要对ngx_cycle_t结构进行
	...
	//若有信号，则进入ngx_signal_process()处理,让master进程处理信号后，程序退出
    if (ngx_signal) 
	{
...
需要打开的监听端口，新的Nginx进程会通过ngx_add_inherited_sockets方法来使用已经打开的TCP监听端口
 例如，为调试方便，设环境变量NGINX为如下值:export NGINX="16000:16500:16600;"
 */
static ngx_int_t ngx_add_inherited_sockets(ngx_cycle_t *cycle)
{
    ...
    if (ngx_array_init(&cycle->listening, cycle->pool, 10,sizeof(ngx_listening_t))!= NGX_OK) /* 初始化ngx_cycle.listening数组, 10个元素空间 */
    {
        return NGX_ERROR;
    }
    ...
    for (p = inherited, v = p; *p; p++) {
        if (*p == ':' || *p == ';') {//sockets以':'或者';'隔开 
            s = ngx_atoi(v, p - v);//sockets是10进制正整数,例如此时解析出s=16000
            if (s == NGX_ERROR) {
...
/* cycle通常是刚刚分配的ngx_cycle_t结构体指针，仅用于传递配置文件的路径信息
用运行Nginx时可能携带的目录参数来　初始化cycle，包括初始化运行目录、配置
目录，并生成完整的nginx.conf配置文件路径*/
static ngx_int_t ngx_process_options(ngx_cycle_t *cycle)
{
    u_char  *p;
    size_t   len;
