...
ngx_int_t ngx_open_listening_sockets(ngx_cycle_t *cycle)
{
    ...
    for (tries = 5; tries; tries--) {//尝试5次连接
        failed = 0;
        ...
        for (i = 0; i < cycle->listening.nelts; i++) {
            ...
            if (ls[i].inherited) {//当前监听句柄来自前一个进程
...
//从连接池中获取一个ngx_connection_t结构体，同时获取相应的读写事件
ngx_connection_t * ngx_get_connection(ngx_socket_t s, ngx_log_t *log)
{
	...
	//将instance标志位置为原来的相反值，这样，当这个ngx_connection_t连接重复使用时，它的instance标志位一定是不同的
...
void ngx_close_connection(ngx_connection_t *c)
{
	...
	/* 调用ngx_del_conn宏或者ngx_del_event宏将读写事件从epoll中移除 */
    if (ngx_del_conn) 
	{
        ngx_del_conn(c, NGX_CLOSE_EVENT);
    }
	...
	/* 如果这个事件在ngx_posted_accept_events或者ngx_posted_events队列中，还需要调用ngx_delete_posted_event宏把事件从post事件队中移除*/
    ...
    ngx_free_connection(c);//把表示连接的ngx_connection_t结构体归还给ngx_cycle_t核心结构体的空闲连接池free_connections
    fd = c->fd;
    c->fd = (ngx_socket_t) -1;
    ...
    ngx_uint_t port)
{
    ...
    {
        len = NGX_SOCKADDRLEN;
        //如果服务器绑定的是通配符IP，即INADDR_ANY，则只有在连接建立后，调用getsockname才能获得本地套接字
