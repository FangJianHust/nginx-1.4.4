...
struct ngx_log_s
{
    ngx_uint_t           log_level;//日志级别或日志类型
    ngx_open_file_t     *file;//日志文件

    ngx_atomic_uint_t    connection;//连接数，不为0时输出到日志中

	//记录日志的回调方法，当handler已经实现且不是DEBUG调试级别时，才会调用handler钩子方法
    ngx_log_handler_pt   handler;
    void                *data;//每个模块都可以自定义data的使用方法。通常，data参数都是在实现了上面的handler回调方法时才使用的。例如，HTTP框架就定义了handler方法，并在data中放入了这个请求的上下文信息，这样每次输出日志时都会把这个请求URI输出到日志的尾部
    ...
    char                *action;//表示当前的动作。实际上，action与data是一样的，只有在实现了handler回调方法时才会使用。例如，HTTP框架就在handler方法中检查action是否为NULL，如果不为NULL，就会在日志后加入"while"+action,以此表示当前日志是在进行什么操作，帮助定位问题
};
...
/* 这里的level不再表示级别(已经是DEBUG级别),而是日志类型,当HTTP模块调用ngx_log_debug宏记录日志时，传入的level参数是NGX_LOG_DEBUG_HTTP,这时如果log参数不属于HTTP模块，如使用了event事件模块的log，则不会输出任何日志，如果是，则输出。 */
